...
The shared request/response contract lives in [docs/API_CONTRACT.md](docs/API_CONTRACT.md). This README does not replace that document.
...
- Inventory on hand is calculated at query time from the event log, with **case-insensitive, whitespace-normalized** item matching:
  ...
  There is no separate inventory table. Item names that are also parser keywords (for example `today`, `left`, `cost`) are not treated as product names.
